...
function MeetingForm({setFormValue}) {
    ...
    const [duration,setDuration]= useState<string | undefined>("30");;
  ...
  return (
    <div className='p-8'>
      ...
      <div className='flex flex-col gap-3 my-4'>
            ...
            <DropdownMenu>
                <DropdownMenuTrigger><Button variant="outline" className='max-w-40'>{duration} min</Button></DropdownMenuTrigger>
                <DropdownMenuContent>
              ...
              {LocationOption.map((option, index) => (
                <div
                  ...
                  className={`border flex flex-col 
                    justify-center items-center p-3 rounded-lg cursor-pointer 
                    hover:bg-blue-100 hover:border-primary ${
                      locationType === option.name ? 'bg-blue-200 border-primary' : ''
                    }`}
            ...
            {locationType && (
              <div className="mt-4">
                <h3 className="text-lg">Selected Location: {locationType}</h3>
              </div>
            )}
            {locationType &&<>
